def solve(board, debug_level):
    """Use a depth-first search to solve the Sudoku puzzle.
    ...
    while len(stack) > 0:
        ...
        if repr(curr) in seen or curr.contains_empty():
            if curr.contains_empty():
                leaves += 1
            else:
                cross_overs += 1
            continue
            ...
            boards = curr.get_possible_boards(*curr.get_minimal_possibilities_square())
            stack.extend(boards)
            seen.add(repr(curr))
...
def read_input_from_stdin(board):
    ...
    while lines < 9:
        ...
        if check_line(line):
            ...
            print("Error while parsing line, expecting nine values, 1-9 or '-' separated by spaces, please try again")
...
def check_line(line):
    ...
    for val in vals:
        ...
        if not (val.isdigit() or val == "-"):
            return False
        if val.isdigit() and not 1 <= int(val) <= 9:
            return False
...
def store_line(line, row, board):
    ...
    for col, val in enumerate(line.split()):
        if val.isdigit():
            board.set_value(row, col, int(val))
